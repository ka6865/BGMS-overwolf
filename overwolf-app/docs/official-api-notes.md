...
검증 결과 (운영, 2026-08-01, BGMS PR #131 머지 후):

`https://bgms.kr/replay/3d?matchId=3462da2c-...&nickname=carpe_diem_kst&platform=steam&t=420` 으로 진입해 확인했다.

- 타임라인 슬라이더 값이 `420000`ms 로 정확히 맞았다 (요청 420초, 전체 `1667712`ms)
- 화면에 `ELAPSED 07:00`, `ALIVE 69`, `자기장 수축 중` 이 표시되고 아군 스쿼드와 적군 96명이 렌더됐다
- 로드 실패 문구 없음
