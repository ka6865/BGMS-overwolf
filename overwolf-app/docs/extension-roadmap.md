...
| FTUE (첫 사용자 경험) | **2026-08-01 구현** | 첫 화면에 2단계 안내를 띄우고 설정이 갖춰지면 숨긴다 |
...
설치 직후 첫 화면이 설정 패널이라 무엇을 해야 하는지 알 수 없었다. 전송을 켜고 닉네임을 넣어야 가치가 생기는 구조인데 그 안내가 없었다.

- hero 아래에 2단계 안내를 띄운다. 1단계는 남은 작업에 따라 문구가 바뀐다(전송 꺼짐이면 "전송을 켜고 닉네임 입력", 전송만 켰으면 "닉네임 입력")
- `canSendHandoff` 가 true 가 되면 안내를 숨긴다. 별도 "다시 보지 않기" 플래그를 두지 않은 이유는 설정을 되돌린 사용자에게 다시 안내가 필요하기 때문이다
- 전송을 켜지 않아도 오버레이는 동작한다는 점과 단축키 위치를 함께 알린다
